...
namespace FaruSneaker.Object
{
    public partial class Payment_co : UserControl
    {
        ...
        {
            DataTable dataTable = bl.search(txt_Search.Text);
            if (dataTable.Rows.Count == 1 )
            {
                DataRow row = dataTable.Rows[0];
                rtx_BillID.Text = row[0].ToString();
                cbx_CusID.SelectedItem = row[1].ToString();
                cbx_StaffID.SelectedItem = row[3].ToString();
                rtx_TotalCash.Text = row[2].ToString();
            }
        }
    }
}
